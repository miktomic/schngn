...
- Empty guest state: the app offers `Add new trip` and keeps the timeline absent until a trip exists. Evidence: [empty state](alternative-usability-review-assets/initial-app.png).
- Three labelled stays: France 1–15 Apr (15 days), Italy 1–10 May (10 days), Spain 1–30 Oct (30 days). At 30 Oct the result is `37 / 90`; the timeline and ledger agree. Evidence: [mobile three-trip view](alternative-usability-review-assets/mobile-three-trips.png), [desktop three-trip view](alternative-usability-review-assets/desktop-three-trips.png).
- Overlapping history: 1–15 Apr, 10–20 Apr, and 18 Apr–2 May. The screen reports `32 / 90`, so overlapping days are deduplicated. Evidence: [overlapping mobile view](alternative-usability-review-assets/mobile-overlap.png).
- Over-limit state: a 1 Jun–10 Sep stay produces the first over-limit result on 30 Aug, `91 / 90`, and a latest safe exit of 29 Aug. Evidence: [risk viewport](alternative-usability-review-assets/mobile-risk-viewport.png).
- Open-ended stay: with prior 1–15 Apr and 1–10 May history plus a 1 Sep stay with no exit date, the screen reports `35 / 90` on 10 Sep. The `35` includes the 25 earlier days plus 10 days from the ongoing stay. It labels the trip as open-ended/projected and extends the visual forecast axis to 9 Mar 2027. Evidence: [ongoing stay](alternative-usability-review-assets/mobile-ongoing.png).
- Explainer checkpoints: baseline `25 / 90`, checked what-if `26 / 90`, planned exit `95 / 90`, and Today `26 / 90`. Evidence: [explainer baseline](alternative-usability-review-assets/explainer-baseline.png), [explainer what-if](alternative-usability-review-assets/explainer-whatif.png), [explainer planned exit](alternative-usability-review-assets/explainer-planned-viewport.png).
...
**User impact:** A traveler can read the date they typed as the date being checked while the large answer still belongs to a different date. On a safety-sensitive calculator, that is a direct answer-to-input mismatch. The same behavior was observed for a date after the allowed maximum (`01/01/2027`). Evidence: [invalid checking-date control](alternative-usability-review-assets/mobile-invalid-checking-date.png).
...
**User impact:** The user cannot reliably tell which marker corresponds to which calendar date or distinguish a close trip from an overlapping one. They must leave the visualization and inspect the cards, which defeats the purpose of the primary data view for dense histories. Evidence: [overlap evidence](alternative-usability-review-assets/mobile-overlap.png).
...
**User impact:** A first-time user can interpret `102 days` as the number counted in the current 180-day window, then see a conflicting `91 / 90` below. The data is correct in the tested scenario; the issue is the visual relationship between two different measures. Evidence: [risk evidence](alternative-usability-review-assets/mobile-risk-viewport.png).
...
**User impact:** The visual weight of a projected continuation can make a forecast look like a booked or confirmed travel record. The latest safe exit (29 Nov in the tested state) and the projected extent need a stronger visual distinction when both are on screen. Evidence: [ongoing evidence](alternative-usability-review-assets/mobile-ongoing.png).
...
**User impact:** Repetition helps scanning but weakens hierarchy. Users may not know which total is the editable inspection and which is the saved result, especially after scrubbing. The labels provide context, but the three numerically identical totals compete with the single primary answer. Evidence: [three-trip mobile view](alternative-usability-review-assets/mobile-three-trips.png).

...
- `bun run test:e2e` runs Playwright mobile Chromium smoke for `/`, `/accuracy`, `/explainer`, `/faq`, `/contact`, and `/app`, including inclusive Schengen trip-planning title/meta/hero/CTA coverage, accuracy-page title/official-source/case/non-endorsement coverage, service-worker install/control plus offline `/app` reload coverage, the canonical Timeline and Trips sections, a dedicated localized Explainer rendered with production components, a sourced localized FAQ, the contact-form field/privacy boundary, expandable saved-trip adjusters, trip validation, reload persistence, JSON export/import, Clerk signup entry points, keyboard focus reachability, and privacy-network assertions.

...
- **Verification:** calendar model tests cover Monday-first grids, leap years, reverse ranges, month clamping, inclusive counts, localized numerals and singular copy, and every locale. Playwright covers reverse cross-month drag through saved-trip output, real touchscreen tap-tap, keyboard selection, mobile month-boundary focus, native-input resynchronization, canceled-drag rollback, 320 px containment, RTL navigation and arrow-key movement, internal dialog scrolling, open-ended stays, PWA/offline behavior, account sync, outside-Schengen breaks, and inline editing. `bun run test:e2e` passed 34/34 and root `bun run check` passed 432 tests plus every typecheck, build, and compiled-agent smoke.

...
export type AnalyticsSource = 'app' | 'dashboard' | 'trip_form' | 'planner' | 'report' | 'waitlist' | 'privacy' | 'manual';
...
export function trackAnalyticsEvent(name: AnalyticsEventName, props: AnalyticsProps = {}, targetWindow: Window | undefined = globalThis.window): AnalyticsEvent {
  ...
  plausible?.(event.name, Object.keys(event.props).length > 0 ? { props: event.props } : undefined);
  return event;
}

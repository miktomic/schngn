  ...
  privacy: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          'Sul sito di produzione, Plausible può ricevere eventi inseriti in una lista consentita, come visualizzazione della pagina, avvio del calcolatore, viaggio aggiunto, simulazione eseguita e interesse per lo sblocco, insieme a categorie generali quali fascia del numero di viaggi, esito, fascia del margine di sicurezza, fonte o fascia di prezzo di prova. SCHNGN rimuove stringhe di query e frammenti degli URL e vieta date, etichette, paesi e sequenze temporali dei viaggi, scelta del passaporto, e-mail e identificatori degli account. Plausible Analytics è configurato senza cookie analitici né tracciamento automatico di moduli, download e link in uscita. Plausible può comunque trattare normali informazioni di rete per produrre statistiche aggregate.'
        ]
      },
      {
        id: 'support-security',
  ...
  privacy: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          'На робочому сайті Plausible може отримувати лише дозволені події, наприклад перегляд сторінки, запуск калькулятора, додавання подорожі, запуск симуляції та інтерес до розблокування, а також узагальнені категорії: діапазон кількості подорожей, результат, діапазон безпечного запасу, джерело або тестову цінову групу. SCHNGN видаляє рядки запитів і фрагменти URL та забороняє передавати дати, назви, країни й хронологію подорожей, вибір паспорта, адреси електронної пошти та ідентифікатори облікових записів. Plausible Analytics налаштовано без аналітичних cookie-файлів і без автоматичного відстеження форм, завантажень і вихідних посилань. Водночас Plausible може обробляти звичайну мережеву інформацію для створення агрегованої статистики.'
        ]
      },
      {
        id: 'support-security',

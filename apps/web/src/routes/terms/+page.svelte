<script lang="ts">
  ...
  import { termsUi } from '$lib/i18n/legalUi';

  let locale = $derived(localeFromPath(page.url.pathname));
  let copy = $derived(termsUi(locale));
  let canonicalUrl = $derived(`https://schngn.com${localizedPath('/terms', locale)}`);
</script>
...
<svelte:head>
  ...
  {#each SUPPORTED_LOCALES as alternateLocale}
    <link rel="alternate" hreflang={alternateLocale} href={`https://schngn.com${localizedPath('/terms', alternateLocale)}`} />
  {/each}
  <link rel="alternate" hreflang="x-default" href="https://schngn.com/terms" />
